...
public class Main {
    ...
    static int getK(int n, char[]s){
        int[] a = new int[n];
        Arrays.fill(a, INF);
        int ans=0;
        a[n-1]=s[n-1]=='0'?0:(10 - (s[n-1]-'0'));

        for(int i =n-2; i>=0; i--){
            int ch = s[i]=='0'?0:(10 - (s[i]-'0'));
            if(ch>=a[i+1]%10)a[i] = a[i+1] + ch - a[i+1]%10;
            else a[i] = a[i+1] + ch - a[i+1]%10 + 10;
        }
        return a[0];
    }
